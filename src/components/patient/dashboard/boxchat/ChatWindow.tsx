...
interface ChatWindowProps {
  messages: { text: string; sender: "A" | "user" }[];
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ messages, onSend, onClose }) => {
  ...
  return (
    <Paper sx={{ position: "fixed", bottom: 70, right: 20, width: { xs:"90vw", sm:400, md:450 }, height: 500, display: "flex", flexDirection: "column", borderRadius:2, overflow:"hidden", zIndex:1000 }} elevation={6}>
      ...
      <Box sx={{ display:"flex", p:1, borderTop:"1px solid #ccc" }}>
        <TextField fullWidth size="small" placeholder="Nhập tin nhắn..." value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") handleSend(); }} />
