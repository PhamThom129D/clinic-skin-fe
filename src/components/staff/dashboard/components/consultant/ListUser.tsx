...
export default function StaffChatInbox() {
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => client.subscribe("/topic/inbox-updates", () => fetchInbox()),
    });
    client.activate();
    ...
    <div style={{ display: "flex", gap: 20, height: "80vh" }}>
      <div style={{ width: 300, overflowY: "auto", border: "1px solid #ccc", borderRadius: 6, padding: 8 }}>
        ...
        {conversations.map((conv) => {
          ...
          return (
            <div key={conv.key} onClick={() => { setActiveKey(conv.key); setUnread(prev => ({ ...prev, [conv.key]: false })); }} style={{ marginBottom: 8, padding: 8, borderRadius: 6, cursor: "pointer", background: activeKey === conv.key ? "#eef" : "#fff", position: "relative", border: "1px solid #ddd" }}>
              <div style={{ fontWeight: "bold" }}>{conv.customerName}</div>
              <div style={{ fontSize: 12, color: "#555" }}>Tin nhắn cuối: {lastMsg}</div>
              {unread[conv.key] && <span style={{ position: "absolute", top: 8, right: 8, background: "red", color: "#fff", borderRadius: "50%", padding: "2px 6px", fontSize: 10 }}>Mới</span>}
            </div>
          );
        })}
      </div>
      <div style={{ flex: 1, border: "1px solid #ccc", borderRadius: 6, overflow: "hidden" }}>
        {activeConv ? <StaffChatWindow conversation={activeConv} staffId={staffId} /> : <div style={{ padding: 20 }}>👉 Chọn khách để chat</div>}
      </div>
    </div>
  );
}
