...
const DoctorAppointmentTable: React.FC<DoctorAppointmentTableProps> = ({ darkMode = false }) => {
  ...
  const columns = useMemo<MRT_ColumnDef<AppointmentResponse>[]>(
    () => [
      { accessorKey: "stt", header: "STT", size: 50, enableSorting: false, Cell: ({ row }) => row.index + 1 },
  ...
  return (
    <>
      <Paper sx={{ p: 2, borderRadius: 3, background: darkMode ? "#1e1e2f" : "#fff" }}>
        ...
        <MaterialReactTable
          ...
          enableSorting
          enablePagination
          muiTablePaperProps={{ sx: { backgroundColor: darkMode ? "#2e2e3e" : "#fff", color: darkMode ? "#f0f0f0" : "#000" } }}
          muiTableBodyRowProps={{ sx: { "&:hover": { backgroundColor: darkMode ? "#3a3a4a" : "#f5f5f5", cursor: "pointer" } } }}
          muiTableBodyCellProps={{ sx: { color: darkMode ? "#f0f0f0" : "#000" } }}
          muiTableHeadCellProps={{ sx: { fontWeight: "bold", backgroundColor: darkMode ? "#2a2a3a" : "#f0f0f0", color: darkMode ? "#f0f0f0" : "#000" } }}
          muiSearchTextFieldProps={{ placeholder: " Tìm kiếm...", variant: "outlined", size: "small", sx: { borderRadius: 2 } }}
